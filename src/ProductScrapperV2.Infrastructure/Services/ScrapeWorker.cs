...
public class ScrapeWorker : BackgroundService
{
    ...
    private static async Task<ScrapeResultDto?> FindBestMatchAsync(
        string productName,
    ...
    {
        ScrapeResultDto? best = null;
        foreach (var link in candidateLinks)
        {
            var rawElements = await scrapingService.ExtractRawElementsAsync(link, cancellationToken);
            var analysis = await chatGpt.AnalyzeProductAsync(productName, link, rawElements, cancellationToken);
            if (analysis is null)
            {
                continue;
            }
            ...
            if (best is null ||
                analysis.MatchPercentage > best.MatchPercentage ||
                (analysis.MatchPercentage == best.MatchPercentage && analysis.ConfidenceScore > best.ConfidenceScore))
            {
                best = analysis;
            }
        }
